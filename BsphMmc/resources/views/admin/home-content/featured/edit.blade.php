...
@section('title', 'Edit Featured Slide')
@section('page-title', 'Edit Featured Slide')

@section('content')
<div class="py-6 max-w-4xl">
    <form method="POST" action="{{ route('admin.home-content.featured.update', $featured) }}" enctype="multipart/form-data" class="space-y-6">
        @csrf
        @method('PUT')
        ...
        <div class="bg-white dark:bg-gray-800 rounded-xl shadow-sm border border-gray-200 dark:border-gray-700 p-6 space-y-5">
            ...
            <div>
                ...
                <input type="text" name="title" value="{{ old('title', $featured->title) }}"
            ...
            <div>
                ...
                <input type="text" name="subtitle" value="{{ old('subtitle', $featured->subtitle) }}"
            ...
            <div>
                ...
                <textarea name="description" rows="5"
                          class="rich-editor w-full px-4 py-2.5 text-sm border border-gray-300 dark:border-gray-600 rounded-lg bg-white dark:bg-gray-700 text-gray-900 dark:text-white focus:ring-2 focus:ring-blue-500 outline-none resize-y">{{ old('description', $featured->description) }}</textarea>
            </div>
        </div>
        ...
        <div class="bg-white dark:bg-gray-800 rounded-xl shadow-sm border border-gray-200 dark:border-gray-700 p-6 space-y-5">
            ...
            <!-- Image Upload -->
            <div>
                <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Slide Image</label>
                ...
                <p class="text-xs text-gray-500 dark:text-gray-400 mt-1">Max 5MB. Recommended size: 1200×600px</p>

                @if($featured->image)
                    <div class="mt-3">
                        <img src="{{ asset('storage/' . $featured->image) }}" alt="{{ $featured->title }}" class="w-full max-h-64 object-cover rounded-lg border border-gray-200 dark:border-gray-600">
                        <p class="text-xs text-gray-600 dark:text-gray-400 mt-2">Currently set</p>
                    </div>
                @endif
            </div>
        </div>
        ...
        <div class="bg-white dark:bg-gray-800 rounded-xl shadow-sm border border-gray-200 dark:border-gray-700 p-6 space-y-5">
            ...
            <div>
                ...
                <input type="text" name="button_text" value="{{ old('button_text', $featured->button_text) }}"
                       class="w-full px-4 py-2.5 text-sm border border-gray-300 dark:border-gray-600 rounded-lg bg-white dark:bg-gray-700 text-gray-900 dark:text-white focus:ring-2 focus:ring-blue-500 outline-none"
            ...
            <div>
                ...
                <input type="text" name="button_link" value="{{ old('button_link', $featured->button_link) }}"
                       class="w-full px-4 py-2.5 text-sm border border-gray-300 dark:border-gray-600 rounded-lg bg-white dark:bg-gray-700 text-gray-900 dark:text-white focus:ring-2 focus:ring-blue-500 outline-none"
                       placeholder="e.g., /news or https://example.com">
            </div>
        </div>
        ...
        <div class="bg-white dark:bg-gray-800 rounded-xl shadow-sm border border-gray-200 dark:border-gray-700 p-6 space-y-5">
            ...
            <div>
                ...
                <input type="number" name="display_order" value="{{ old('display_order', $featured->display_order) }}" min="0"
            ...
            <div class="flex items-center gap-3">
                <input type="checkbox" name="status" value="1" {{ old('status', $featured->status) ? 'checked' : '' }}
        ...
        <div class="flex items-center gap-3">
            ...
            <form method="POST" action="{{ route('admin.home-content.featured.destroy', $featured) }}" class="inline"
                  onsubmit="return confirm('Are you sure you want to delete this slide?');">
                @csrf
                @method('DELETE')
                <button type="submit" class="px-4 py-2.5 bg-red-100 dark:bg-red-900 text-red-700 dark:text-red-200 font-medium rounded-lg hover:bg-red-200 dark:hover:bg-red-800 transition">
