...
@section('title', $officeName . ' · ' . ($project ? 'Edit' : 'Add') . ' Project')

@section('content')
<div class="py-6">
    <div class="max-w-4xl mx-auto space-y-6">
        <div>
            <h1 class="text-2xl font-semibold text-gray-900 dark:text-white">{{ $officeName }} · {{ $project ? 'Edit' : 'Add' }} Project</h1>
            <p class="text-sm text-gray-500 dark:text-gray-400">{{ $project ? 'Update' : 'Create a new' }} project for {{ $officeName }}.</p>
        </div>
        ...
        <div class="bg-white dark:bg-gray-800 rounded-xl shadow-sm border border-gray-200 dark:border-gray-700 p-6">
            ...
            <form action="{{ $project ? route('admin.offices.generic.projects.update', ['office' => $office, 'officeProject' => $project]) : route('admin.offices.generic.projects.store', ['office' => $office]) }}" method="POST" enctype="multipart/form-data" class="space-y-6">
                @csrf
                @if($project)
                    @method('PUT')
                @endif
                ...
                <div class="flex justify-end gap-3">
                    <a href="{{ route('admin.offices.generic.projects', ['office' => $office]) }}" class="px-4 py-2 border border-gray-300 rounded-lg text-sm text-gray-700 dark:text-gray-200 hover:bg-gray-100 dark:hover:bg-gray-700">Cancel</a>
