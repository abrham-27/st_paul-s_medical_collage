...
import { apiService, type PublicHealthPartnershipListItem } from '../services/api';
import '../school-of-medicine/Overview.css';
import '../school-of-medicine/Partnership.css';

type AreaFilter = 'all' | 'local' | 'international';

export default function PublicHealthPartnershipPage({ onBack }: { onBack: () => void }): JSX.Element {
    const [partnerships, setPartnerships] = useState<PublicHealthPartnershipListItem[]>([]);
    const [loading, setLoading] = useState(true);
    const [error, setError] = useState<string | null>(null);
    const [areaFilter, setAreaFilter] = useState<AreaFilter>('all');

    useEffect(() => {
        apiService.getPublicHealthPartnerships()
            ...
            <div className="hero-section">
                <div className="container">
                    ...
                    <div className="hero-content">
                        <span className="badge">School of Public Health</span>
                        <h1>Partnership & Collaboration</h1>
                        <p className="lead">Explore our local and international partnerships advancing public health education and research.</p>
                    </div>
                </div>
            </div>
                        ...
                        {filtered.map(item => (
                            <article key={item.id} className="partnership-list-card">
                                ...
                                <div className="partnership-list-card-body">
                                    ...
                                    <Link to={`/academics/public-health/partnership-collaboration/${item.slug}`} className="partnership-view-detail">
                                        View detail →
                                    </Link>
                                </div>
                            </article>
