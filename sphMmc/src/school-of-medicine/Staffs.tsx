...
export default function MedicineStaffs({ onBack, onViewProfile }: Props): JSX.Element {
  const [staffs, setStaffs] = useState<AcademicStaffMember[]>([]);
  const [loading, setLoading] = useState(true);
  const [search, setSearch] = useState('');

  useEffect(() => {
    apiService.getAcademicStaffs('medicine')
      ...
      <div className="staffs-hero">
        <div className="staffs-container">
          ...
          <div className="staffs-hero-content">
            ...
            <p className="lead">Meet the dedicated faculty shaping the future of medicine at SPHMMC.</p>
          </div>
        </div>
      </div>
